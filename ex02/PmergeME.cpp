    ...
    while (pair_size > 0)
    {
        ...
        if (nb > *mid)
        {
            ...
            if (is_pair_ratio(top, pair_ratio, vec) == 1 && pair_ratio != 1)
                ...
                *vec_it++;
            if (is_pair_ratio(top, pair_ratio, vec) == 1 && nb > *top)
            {
                std::cout << "je suis au max et je dois mettre mon nb en dernier" << std::endl;
                vec_it++;
            }
            std::cout << "vec_it dans insertion " << *vec_it << std::endl;
        }
        else if (nb <= *mid)
        {
            ...
            if (nb > *bot)
            {
                std::cout << "je passe par la valeur de mid" << std::endl;
                while (vec_it < mid)
                    vec_it++;
            }
            else
            {
                std::cout << "je passe par la valeur de bot" << std::endl;
                while (vec_it < bot)
                    vec_it++;
            }
            if (*bot_verif > nb)
        ...
        // size_t tmp = pair_ratio;
        if (*top < nb)
        {
            vec.insert(vec.end(), 1, *buf_it);
            buf_it = vec_buf.erase(buf_it);
        }
        for(size_t i = pair_ratio; i > pair_ratio; i++)
        {
            std::cout << "valeur de vecteur " << *buf_it << std::endl;
